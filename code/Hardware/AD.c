...
void AD_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PA2、PA3、PB0和PB1引脚初始化为模拟输入
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_8, 3, ADC_SampleTime_55Cycles5);	//规则组序列3的位置，配置为通道8
	ADC_RegularChannelConfig(ADC1, ADC_Channel_9, 4, ADC_SampleTime_55Cycles5);	//规则组序列4的位置，配置为通道9
